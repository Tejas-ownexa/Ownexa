...
def get_general_ledger_dashboard(current_user):
    ...
    try:
        ...
        # Calculate total assets (sum of all asset entries - debits increase assets, credits decrease)
        asset_debits = sum(float(entry.amount) for entry in all_ledger_entries 
                          if entry.account_category == 'assets' and entry.transaction_type == 'debit')
        asset_credits = sum(float(entry.amount) for entry in all_ledger_entries 
                           if entry.account_category == 'assets' and entry.transaction_type == 'credit')
        current_assets = asset_debits - asset_credits
        
        # Calculate total liabilities (sum of all liability entries - credits increase liabilities, debits decrease)
        liability_debits = sum(float(entry.amount) for entry in all_ledger_entries 
                              if entry.account_category == 'liabilities' and entry.transaction_type == 'debit')
        liability_credits = sum(float(entry.amount) for entry in all_ledger_entries 
                               if entry.account_category == 'liabilities' and entry.transaction_type == 'credit')
        current_liabilities = liability_credits - liability_debits
        ...
        # Calculate total revenue (sum of all revenue entries)
        revenue_debits = sum(float(entry.amount) for entry in all_ledger_entries 
                            if entry.account_category == 'revenue' and entry.transaction_type == 'debit')
        revenue_credits = sum(float(entry.amount) for entry in all_ledger_entries 
                             if entry.account_category == 'revenue' and entry.transaction_type == 'credit')
        current_revenue = revenue_credits - revenue_debits
        
        # Calculate total expenses (sum of all expense entries)
        expense_debits = sum(float(entry.amount) for entry in all_ledger_entries 
                            if entry.account_category == 'expenses' and entry.transaction_type == 'debit')
        expense_credits = sum(float(entry.amount) for entry in all_ledger_entries 
                             if entry.account_category == 'expenses' and entry.transaction_type == 'credit')
        current_expenses = expense_debits - expense_credits
        ...
        # Calculate previous month assets
        prev_asset_debits = sum(float(entry.amount) for entry in prev_month_entries 
                               if entry.account_category == 'assets' and entry.transaction_type == 'debit')
        prev_asset_credits = sum(float(entry.amount) for entry in prev_month_entries 
                                if entry.account_category == 'assets' and entry.transaction_type == 'credit')
        prev_assets = prev_asset_debits - prev_asset_credits
