...
def create_and_send_envelope_with_embedded_recipient(
    ...
    # Add anchors to the PDF like: *signHereTenant*, *dateSignedTenant*, *fullNameTenant*, *initialsTenant*
    sign_here = SignHere(
        recipient_id='1',
        document_id='1',
    ...
    initials_here = None
    try:
        from docusign_esign import DateSigned, FullName, InitialHere
    ...
    tabs = Tabs(
        ...
        date_signed_tabs=[date_signed] if date_signed else None,
        full_name_tabs=[full_name] if full_name else None,
        initial_here_tabs=[initials_here] if initials_here else None,
    )
    ...
    # Note: For embedded signing, the email and name must match signer, and client_user_id must match above
...
def download_completed_document(envelope_id: str, output_path: str) -> str:
    """
    Downloads the combined PDF for a completed envelope and writes to output_path.
    Returns the saved path.
    """
    api_client, account_id = _get_api_client()
    envelopes_api = EnvelopesApi(api_client)

    # DocuSign "combined" document id is 'combined'
